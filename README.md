...
**ただしこれは実行順の強制ではない。** どのスキルも単体で使える——`code-reviewer` を要件定義を経ずに呼んでも動く。この図が言っているのは「上流を飛ばした場合、下流のスキルはその前提を欠いたまま動く」ということだけで、たとえば `code-reviewer` なら要件充足の判定を諦めて残りの観点でレビューする（推測で埋めずに `[ask]` で返す）。必須ゲートは1. の1組だけ。
